...
Bootstrap is a one-time host-admin command that creates the first owner/workspace and emits a credential once to a protected destination. Disable it after success. Normal operations then use authenticated interfaces; no public unauthenticated bootstrap route.
...
HTTP MCP is a protected resource. Publish authorization-server and protected-resource metadata, validate issuer/audience/expiry/scopes, and authorize every request. Proposed machine access uses pre-registered Better Auth OAuth clients with explicit machine scopes and short-lived tokens. Better Auth documents a client-credentials grant, but that does not imply every MCP client can acquire those tokens. [OAuth provider](https://better-auth.com/docs/plugins/oauth-provider).

The MCP authorization specification provides OAuth discovery and resource/audience rules. Version negotiation and interoperability must be tested against actual clients. Headless v1 promises configured token-capable HTTP clients plus stdio compatibility. An arbitrary client demanding interactive authorization-code login/consent is outside that promise; future support can integrate an existing identity provider instead of adding a product dashboard. [MCP authorization](https://modelcontextprotocol.io/specification/2026-07-28/basic/authorization).
