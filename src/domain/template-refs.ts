...
export function throwTemplateRefIssues(issues: TemplateRefIssue[]): never {
  ...
  throw new ReflowError(
    ...
    false,
    {
      hint: 'Create/publish the missing templates with `reflow template push <file.tsx>` (renders locally, uploads HTML) or template.create (sourceKind=html) + template.publish, then set each email.send node input.templateVersionId.literal to the published version id.',
      details: {
        issues,
        nextSteps: [
