...
export function reflowOAuthOptions(config: Config): OAuthOptions<Scope[]> {
  return {
...
export function createAuth(config: Config, pool: Pool) {
  const oauthOptions = reflowOAuthOptions(config);
  ...
  return betterAuth({
    ...
    databaseHooks: {
      user: { create: { after: async (user) => {
        ...
        await pool.query("insert into memberships(workspace_id, user_id, role) select id, $1, 'viewer' from workspaces order by created_at limit 1 on conflict do nothing", [user.id]);
      } } },
    },
    plugins: [
      admin(),
