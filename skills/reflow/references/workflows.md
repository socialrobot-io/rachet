...
5. Call `workflow_validate`. Then call `workflow_simulate` twice: once with `receivedEvents: []`, once with the activation events the product will emit. Use `{ "eventType": "product.activated", "data": { ... } }` when a branch or action reads the payload. Inspect resolved action inputs and both event/timeout scenarios.
...
To read event data, use `event["product.activated"].data.field`. Brackets keep a dotted event name as one key. Pass the whole `data` object to `email.send.input.props` or `contact.update.input.fields` when the template or contact needs it. See [event data](../../../docs/EVENT_DATA.md).
